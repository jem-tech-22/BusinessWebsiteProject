...
function getPromos() {
  return [
    { title: "Student Discount", subtitle: "10% off with a valid student ID", tag: "Always On" },
    { title: "Party Pack", subtitle: "Order 10+ drinks and get 1 free", tag: "Bulk" }
  ];
}
...
function renderMenu(data) {
    ...
    category.items.forEach(item => {
      ...
      it.innerHTML = `
        <div class="item-img-wrap">
          <img src="${item.img}" alt="${escapeHtml(item.name)}" class="item-img" loading="lazy" />
        </div>
        <div class="meta">
          <div>
  ...
  container.addEventListener('click', (e) => {
    ...
    addQuickOrder(`${name} — ₱${formatPrice(price)}`);
  });
}
...
function renderPromos(promos) {
  ...
  promos.forEach(p => {
    ...
    el.className = 'promo';
    el.innerHTML = `<h4>${escapeHtml(p.title)}</h4><p class="muted">${escapeHtml(p.subtitle)}</p><span class="badge">${escapeHtml(p.tag)}</span>`;
